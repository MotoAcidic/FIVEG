...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZVGModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zvgTracker->HasMintTx(hash))) {
            //zVG stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZVG;
...
bool IsZVGType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZVG:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZVG || type == TransactionRecord::MNReward) {
