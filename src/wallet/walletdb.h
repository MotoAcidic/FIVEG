...
public:
    ...
    bool WriteZVGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVGSeed_deprecated(uint256& seed);
    bool EraseZVGSeed();
    bool EraseZVGSeed_deprecated();

    bool WriteZVGCount(const uint32_t& nCount);
    bool ReadZVGCount(uint32_t& nCount);
