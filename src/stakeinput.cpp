...
// The zVG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zVG is a hash of the serial
...
{
    //Create an output returning the zVG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZVGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zVG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zVG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZVGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zVG output", __func__);
...
{
    CzVGTracker* zvgTracker = pwallet->zvgTracker.get();
    CMintMeta meta;
    if (!zvgTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zvgTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a VG stake is the outpoint
