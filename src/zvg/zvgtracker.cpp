...
CzVGTracker::CzVGTracker(std::string strWalletFile)
{
...
CzVGTracker::~CzVGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVGTracker::Init()
{
...
bool CzVGTracker::Archive(CMintMeta& meta)
{
...
bool CzVGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzVGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzVGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzVGTracker::GetSerialHashes()
{
...
CAmount CzVGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVGWallet* zVGWallet)
{
    bool iszVGWalletInitialized = (NULL != zVGWallet);
    CMintMeta meta;
    ...
    if (! iszVGWalletInitialized)
        zVGWallet = new CzVGWallet(strWalletFile);
    meta.isSeedCorrect = zVGWallet->CheckSeed(dMint);
    if (! iszVGWalletInitialized)
        delete zVGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzVGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVGWallet* zVGWallet = new CzVGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zVGWallet);
        }
        delete zVGWallet;
        LogPrint("zero", "%s: added %d dzvg from DB\n", __func__, listDeterministicDB.size());
    }
