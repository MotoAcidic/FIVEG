...
CzVGWallet::CzVGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVGSeed_deprecated()) {
                    LogPrintf("%s: Updated zVG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVGCount(nCountLastUsed);
    else if (!walletdb.ReadZVGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVGWallet::Lock()
{
    seedMaster = 0;
}

void CzVGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVGWallet::LoadMintPoolFromDB()
{
...
void CzVGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvgTracker which also adds to database
    pwalletMain->zvgTracker->Add(dMint, true);
...
void CzVGWallet::SeedToZVG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVGWallet::GenerateDeterministicZVG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzVGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
