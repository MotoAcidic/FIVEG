...
// FIVEG: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_17_COLLATERAL_CHANGE) r = SPORK_17_COLLATERAL_CHANGE_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_17_COLLATERAL_CHANGE") return SPORK_17_COLLATERAL_CHANGE;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_17_COLLATERAL_CHANGE) return "SPORK_17_COLLATERAL_CHANGE";
