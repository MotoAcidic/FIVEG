...
Note
---------------------
Always use absolute paths to configure and compile FIVEG Core and the dependencies,
...
memory available when compiling FIVEG Core. On systems with less, gcc can be
...
above instead of `libssl-dev`. FIVEG Core does not support the use of OpenSSL 1.1, though compilation is still possible
...
To build FIVEG Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build fiveg-qt, make sure that the required packages for Qt development
...
Security
--------
To help make your FIVEG Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, FIVEG Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, FIVEG Core may be compiled in
